...
package net.apucsw.mcrssmod.keybind;
...
import net.apucsw.mcrssmod.procedures.UDS_GUIOnKeyReleasedProcedure;
import net.apucsw.mcrssmod.procedures.UDS_GUIOnKeyPressedProcedure;
...
public class UserDataStatusGUIKeyBinding extends McrssmodModElements.ModElement {
	...
	public UserDataStatusGUIKeyBinding(McrssmodModElements instance) {
		super(instance, 14);
	...
	public void initElements() {
		keys = new KeyBinding("key.mcrssmod.user_data_status_gui", GLFW.GLFW_KEY_HOME, "key.categories.mcrssmod");
		...
		if (Minecraft.getInstance().currentScreen == null) {
			if (event.getKey() == keys.getKey().getKeyCode()) {
				if (event.getAction() == GLFW.GLFW_PRESS) {
					McrssmodMod.PACKET_HANDLER.sendToServer(new KeyBindingPressedMessage(0, 0));
					pressAction(Minecraft.getInstance().player, 0, 0);
					lastpress = System.currentTimeMillis();
				} else if (event.getAction() == GLFW.GLFW_RELEASE) {
					int dt = (int) (System.currentTimeMillis() - lastpress);
					McrssmodMod.PACKET_HANDLER.sendToServer(new KeyBindingPressedMessage(1, dt));
